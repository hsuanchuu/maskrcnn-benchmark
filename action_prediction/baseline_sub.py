...
class baseline(nn.Module):
    ...
    def __init__(self, config, class_num=4, is_cat=False, is_feature=False):
        ...
        self.action_pred = nn.Linear(2048+1024, 4)
    ...
    def forward(self, x):
        ...
        obj = x[:-1]
        glo = x[-1]
        x = self.head(obj) # out: x.shape = (Batchsize, 2048, 14, 14)
        # x = torch.cat((x, glob[0]), dim=1) # out: x.shape = (Batchsize, 2048+1024, 14, 14)
        x = x.transpose(1,0)
        x = self.avgpool1(x) # out: x.shape = (Batchsize, 2048+1024, 1, 1)
        x = x.view(x.size(1), -1)
        glo = self.avgpool2(glo)
        glo = glo.reshape(1, -1)
        x = torch.cat((x, glo), dim=1)
        pred = self.action_pred(x)
